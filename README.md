...
- **Fix race condition in browser singleton lifecycle**: `initialize_browser`/`shutdown_browser` in `delivery/factories.py` mutate `_ref_count`/`_browser_instance` without a lock. Since `mcp_lifespan` runs once per MCP connection (not once per process), two connections arriving concurrently while the browser is down can both pass the `is None` check and each launch a Chromium instance, leaking the first one. Needs an `asyncio.Lock` around both functions.
- **Reclaim resources from crashed/idle connections**: `_ref_count` only reaches zero when a connection's `mcp_lifespan` exits cleanly. A client that crashes without a clean disconnect leaves its `Page`/`BrowserContext` open forever and keeps the shared browser alive indefinitely, even though FastMCP's underlying `session_idle_timeout` (in `StreamableHTTPSessionManager`) is never wired up. Plan: track a last-activity timestamp per `agent_id` in `PlaywrightBrowser`, and run a periodic background task that force-closes tabs idle for longer than a configurable threshold (default 10 minutes).
